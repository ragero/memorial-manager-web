    ...
    validations() {
        return [
            [
                ...
                check('anoPublicacao').trim().not().isInt().withMessage('O ano da publicação deve ser um valor inteiro.'),
                check('paginaInicial').trim().not().isInt().withMessage('A página inicial da publicação deve ser um valor inteiro.'),
                check('paginaFinal').trim().not().isInt().withMessage('A página final da publicação deve ser um valor inteiro.'),
                check('imagem')
                    .custom((value, { req }) => {
